...
def testGenerator(test_path,num_image = 120,target_size = (512,512),flag_multi_class = False,as_gray = False):
...
def labelVisualize(num_class,color_dict,img):
    img = img[:,:,2] if len(img.shape) == 3 else img	#img[:,:,0]
...
def saveResult(save_path,npyfile,flag_multi_class = False,num_class = 2):
    for i,item in enumerate(npyfile):
        img = labelVisualize(num_class,COLOR_DICT,item) if flag_multi_class else item[:,:,2]	#item[:,:,0]
